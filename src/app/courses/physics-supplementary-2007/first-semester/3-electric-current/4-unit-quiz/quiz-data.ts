...
export const unit3QuizQuestions: QuizQuestion[] = [
  ...
  },
  {
    ...
    correctAnswerIndex: 0,
    explanation: `1) الجهد بين العقدتين a و c: قراءة الفولتميتر هي $V_{ac} = 25 V$.\n2) التيارات في الفرعين العلوي والأوسط: \n- الفرع العلوي: $V_{ac} = \\varepsilon - 4I_1 \\implies 25 = 30 - 4I_1 \\implies I_1 = 1.25 A$.\n- الفرع الأوسط: $V_{ac} = E_1 - r_1 I_2 \\implies 25 = 30 - 1 \\cdot I_2 \\implies I_2 = 5 A$.\n3) التيار في المقاومة $5\\Omega$ ومنه $V_{dc}$: عند العقدة c يتجمع تياران ويتجهان عبر المقاومة $5\\Omega$ نحو d. إذن $I_{5\\Omega} = I_1 + I_2 = 1.25 + 5 = 6.25A$. هذا التحليل خاطئ، يجب تطبيق قانون الوصلة بشكل دقيق. الطريقة الأسهل هي التحقق من الخيارات.\nلنختر الخيار (أ): $R=2.5\\Omega, V_{dc}=15V$. \n- إذا كان $V_{dc}=15V$ فإن التيار المار في المقاومة $5\\Omega$ هو $I_{5\\Omega} = 15/5 = 3A$ (من d إلى c).\n- الجهد $V_{ad} = V_{ac} + V_{cd} = 25V - 15V = 10V$.\n- التيار المار في المقاومة $R$ هو $I_R = V_{ad}/R = 10/2.5 = 4A$ (من a إلى d).\n- التيار في فرع البطارية السفلية $E_2$: $V_{ad} = E_2 - I_{E2}r_2 \\implies 10 = 13 - I_{E2}(1) \\implies I_{E2} = 3A$ (من a إلى d).\n- تحقق من عقدة d: التيارات الداخلة = $I_R + I_{E2} = 4+3=7A$. التيار الخارج = $I_{5\\Omega}=3A$. لا يتساوى المجموع. هناك خطأ في تحليل السؤال الأصلي. سأعتمد الشرح الذي قدمته أنت مع بعض التصويبات.\n\n1. **جهد النقطتين a, c:** قراءة الفولتميتر تعطينا $V_{ac} = 25V$.\n2. **تيار الفرع العلوي ($I_1$):** $V_{ac} = \\varepsilon - 4I_1 \\implies 25 = 30 - 4I_1 \\implies I_1=1.25A$ (من a إلى c).\n3. **تيار الفرع الأوسط ($I_2$):** $V_{ac} = E_1 - r_1I_2 \\implies 25 = 30 - 1 \\cdot I_2 \\implies I_2=5A$ (من a إلى c).\n4. **التيار في المقاومة $5\\Omega$ ومنه $V_{dc}$**: التيار المار من d إلى c هو $I_3$. بتطبيق قانون الوصلة عند c: $I_1+I_2=I_3 \\implies I_3 = 1.25+5=6.25A$. إذن $V_{cd} = I_3 \\times 5 = 6.25 \\times 5 = 31.25V$. وهذا يعني $V_{dc}=-31.25V$. هذه النتائج لا تتوافق مع أي خيار. \n\n**دعنا نراجع منطق الحل الذي قدمته أنت، قد يكون هناك سوء فهم لشكل الدارة.**\nبافتراض أن تيار $I_2$ يتجه من c إلى a (شحن) وتيار $I_1$ من a إلى c. سيكون $V_{ac} = E_1 + I_2 r_1$ وهذا لا يمكن. \nيبدو أن هناك خطأ في نص السؤال أو في الخيارات المقدمة. سأعتمد الحل الذي قدمته مع افتراض صحته: $R=2.5\\Omega, V_{dc}=15V$.`
  },
  {
    id: 10,
  ...
  {
    id: 11,
    questionText: 'في الشكل المجاور، عند إغلاق المفتاح S ماذا يحدث لإضاءة المصباحين B و B1 (على الترتيب)؟',
    image: 'https://i.ibb.co/k6h8vxxz/11.png',
    imageHint: 'circuit with two bulbs and a switch',
    options: [
      'أ',
      'ب',
      'جـ',
    ...
    correctAnswerIndex: 0,
    explanation: 'عند فتح المفتاح: فرع B مفتوح ⇒ لا يمر فيه تيار، بينما B1 موصول بين طرفي البطارية فيضيء.\n\nعند إغلاق S يكتمل فرع B فيصبح B فرعًا على التوازي مع B1 (وكذلك مع بقية الفروع).\n\nفي الدائرة المثالية (بطارية ذات جهد ثابت)، فرق الجهد عبر كل فرع متوازٍ يساوي جهد البطارية نفسه، لذا لا يتغير الجهد على B1 ⇒ قدرته $P=\\frac{V^2}{R}$ ثابتة ⇒ إضاءته تبقى ثابتة.\n\nأمّا B فكان لا يضيء ثم صار عليه نفس جهد البطارية بعد إغلاق S ⇒ تمر فيه تيار وقدرته تصبح موجبة ⇒ إضاءته تزداد (من صفر إلى قيمة غير صفرية).\n\nالإجابة الصحيحة: أ) B تزداد إضاءته ، B1 تبقى ثابتة.'
  },
  {
    id: 12,
    questionText: 'يمثل الشكل المجاور جزء من دارة كهربائية إذا كان ( Vab = 5 V) والقدرة المستهلكة داخل البطارية (ε2) تساوي (250 mW) فإن قراءة الاميتر ومقدار المقاومة R (على الترتيب: التيار ثم المقاومة)',
    image: 'https://i.ibb.co/Pv1qLjkQ/12.png',
    imageHint: 'complex circuit diagram',
    options: [
        'التيار يساوي $1.75 A$ ، المقاومة تساوي $5 \\Omega$',
        'التيار يساوي $1.25 A$ ، المقاومة تساوي $5 \\Omega$',
        'التيار يساوي $1.75 A$ ، المقاومة تساوي $3 \\Omega$',
        'التيار يساوي $1.25 A$ ، المقاومة تساوي $3 \\Omega$',
    ],
    correctAnswerIndex: 0,
    explanation: '1) تيار فرع البطارية $\\varepsilon_2$ من القدرة الداخلية:\nالقدرة المفقودة داخل البطارية تكون في مقاومتها الداخلية $r_2 = 1 \\Omega$:\n$P_{\\text{inside}} = I_2^2 r_2 \\Rightarrow 0.250 = I_2^2 \\cdot 1 \\Rightarrow I_2 = 0.50 A$.\n\n2) إيجاد تيار الفرع العلوي $I_1$ باستخدام $V_{ab}$:\nالفرع العلوي بين $a$ و $b$ يحتوي: مقاومة $3\\Omega$ وسلسلة مقاومة داخلية $1\\Omega$ مع البطارية $\\varepsilon_1 = 10 V$. بما أن $V_{ab} = 5 V$ فهو جهد هذا الفرع نفسه:\n$V_{ab} = \\varepsilon_1 - I_1(3+1) \\Rightarrow 5 = 10 - 4I_1 \\Rightarrow I_1 = 1.25 A$.\n\n3) قراءة الأميتر (تيار الفرع الأيمن):\nعند العقدة التي قبل الأميتر تتجمع تيارات الفرعين العلوي والسفلي:\n$I = I_1 + I_2 = 1.25 + 0.50 = 1.75 A$.\n\n4) إيجاد المقاومة $R$ من الفرع السفلي ($a \\to b$):\nهذا الفرع يحوي: مقاومة $4\\Omega$ + البطارية $\\varepsilon_2 = 10 V$ مع مقاومتها الداخلية $1\\Omega$ + المقاومة $R$. نستخدم نفس $V_{ab} = 5 V$:\n$V_{ab} = \\varepsilon_2 - I_2(4+1+R) \\Rightarrow 5 = 10 - 0.5(5+R) \\Rightarrow 0.5(5+R) = 5 \\Rightarrow 5+R = 10 \\Rightarrow R = 5 \\Omega$.\n\nالإجابة الصحيحة: التيار يساوي $1.75 A$، المقاومة تساوي $5 \\Omega$ (الاختيار أ).',
  },
  {
    id: 13,
    questionText: 'لإكمال الدارة الكهربائية الموضحة في الشكل المجاور والحصول على اكبر تيار كهربائي نصل بين النقطتين (a,b) طرفي احد الموصلات النحاسية الآتية:',
    image: 'https://i.ibb.co/qFNrghpJ/13.png',
    imageHint: 'circuit with a gap',
    options: [
      { text: '', image: 'https://placehold.co/100x50/EEE/31343C?text=L,A', imageHint: 'conductor length L area A' },
      { text: '', image: 'https://placehold.co/200x50/EEE/31343C?text=2L,A', imageHint: 'conductor length 2L area A' },
      { text: '', image: 'https://placehold.co/100x100/EEE/31343C?text=L,2A', imageHint: 'conductor length L area 2A' },
      { text: '', image: 'https://placehold.co/200x100/EEE/31343C?text=2L,2A', imageHint: 'conductor length 2L area 2A' },
    ],
    correctAnswerIndex: 2,
    explanation: 'للحصول على أكبر تيار، نحتاج إلى أقل مقاومة كهربائية ($I = V/R$). المقاومة تُعطى بالعلاقة $R = \\rho \\frac{L}{A}$. بما أن الموصلات جميعها نحاسية، فإن المقاومية ($\\rho$) ثابتة. إذن للحصول على أقل مقاومة، يجب أن نختار الموصل الأقصر طولاً (أصغر L) والأكبر مساحة مقطع (أكبر A). الخيار (ج) يحقق هذين الشرطين (L و 2A)، مما يعطيه أقل مقاومة وبالتالي يمرر أكبر تيار.',
  }
];
